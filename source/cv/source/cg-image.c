...
CGImage *CGImage_randomImage(uint32_t width, uint32_t height, uint32_t cpp)
{
	...
	img->data = calloc(width * height * cpp, sizeof(uint8_t));

	srand(time(NULL));

	for (size_t i = 0; i < width * height * cpp; i++)
	{
		img->data[i] = (uint8_t)((rand() / (float)RAND_MAX) * 255.0f);
	}
 ...
 */
void CGImage_pixelAt(CGImage *source, uint32_t i, uint32_t j, uint8_t *c)
{
	assert(source != NULL);
	assert(c != NULL);
...
void CGImage_colorToGrayscale(CGImage *source, CGImage *target)
{
	...
	uint32_t height = (target->height = source->height);
	uint32_t width = (target->width = source->width);
	uint32_t cpp = source->cpp;
...
void CGImage_colorToBW(CGImage *source, CGImage *target, uint8_t threshold)
{
	assert(source != NULL);
	assert(target != NULL);
	assert((source->cpp == 3) || (source->cpp == 4));
...
void CGImage_grayscaleToBW(CGImage *source, CGImage *target, uint8_t threshold)
{
	assert(source != NULL);
	assert(target != NULL);
	assert(source->cpp == 1);
...
void CGImage_toRGB(CGImage *source, CGImage *target)
{
	...
	const uint8_t *sdata = source->data;
...
void CGImage_drawLine(CGImage *source, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint8_t *color, int wd)
{
	assert(source != NULL);
...
void CGImage_drawCircle(CGImage *source, uint32_t xm, uint32_t ym, uint32_t r, uint8_t *color, int wd)
{
